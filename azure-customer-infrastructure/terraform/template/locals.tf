locals {
  ...
  address_space          = ["10.11.xx.0/24"]
  snet-presentation-prod = ["10.11.xx.0/26"]
  snet-dev-test-prod     = ["10.11.xx.0/26"]
  snet-sap-prod          = ["10.11.xx.0/26"]
  snet-db-prod           = ["10.11.xx.0/26"]
  ...
  bpc = {
    dev = {
      subnet_name                  = azurerm_subnet.snet-dev-test-prod.name
      subnet_id                    = azurerm_subnet.snet-dev-test-prod.id
    ...
    prod = {
      subnet_name                  = azurerm_subnet.snet-sap-prod.name
      subnet_id                    = azurerm_subnet.snet-sap-prod.id
      size                         = "Standard_D8s_v5"
      zone                         = "2"
      hana_sid                     = local.hana_prod_sid
      proximity_placement_group_id = azurerm_proximity_placement_group.prod.id
  ...
  vda = {
    dev = {
      ...
      zone     = "1"
      size     = "Standard_D2s_v5"
      image_id = data.azurerm_image.vda2019.id
    }
    prod = {
      name     = "az${local.customer_number}vdadevtemplate"
      zone     = "2"
