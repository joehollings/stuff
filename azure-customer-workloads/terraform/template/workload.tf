...
resource "azurerm_backup_protected_file_share" "shared" {
  resource_group_name       = azurerm_resource_group.prod.name
  recovery_vault_name       = azurerm_recovery_services_vault.this.name
  source_storage_account_id = data.azurerm_storage_account.prod.id
  source_file_share_name    = data.azurerm_backup_protected_file_share.shared.name
  backup_policy_id          = azurerm_backup_policy_file_share.DailyDataBackup.id
}
...
resource "azurerm_backup_protected_file_share" "backup" {
  ...
  recovery_vault_name       = azurerm_recovery_services_vault.this.name
  source_storage_account_id = data.azurerm_storage_account.prod.id
  source_file_share_name    = data.azurerm_backup_protected_file_share.backup.name
  backup_policy_id          = azurerm_backup_policy_file_share.DailyDataBackup.id
}

module "bpc" {
  source                       = "github.com/OpalwaveSolutions/terraform-azure-windows-bpc?ref=v1.3.3"
  for_each                     = local.bpc
