resource "azurerm_resource_group" "prod" {
  name     = "11${local.customer_number}-rsg-prod"
  location = local.location

  tags = {
    customer    = local.customer_name
    environment = "prod"
    project     = "11${local.customer_number}"
  }
}

resource "azurerm_recovery_services_vault" "this" {
  name                = "rsv-11${local.customer_number}-prod"
...
module "primary_dc" {
  source                       = "github.com/OpalwaveSolutions/terraform-azure-windows-dc-cc?ref=v1.2.1"
...
# uncomment this once primary dc is provisioned and customer doamin is configured and push to repo
module "dc_cc" {
  source                       = "github.com/OpalwaveSolutions/terraform-azure-windows-dc-cc?ref=v1.2.1"
...
module "network" {
  ...
  virtual_network_name        = "11${local.customer_number}-vnet-app-${each.key}"
  location                    = azurerm_resource_group.prod.location
  resource_group_name         = azurerm_resource_group.prod.name
...
module "storage" {
  source               = "github.com/OpalwaveSolutions/terraform-azure-storage-account?ref=v1.1.0"
