...
build {
  ...
  provisioner "powershell" {
    script = "./packer/powershell/rsat.ps1"
  }
  provisioner "windows-restart" {
    restart_check_command = "powershell -command \"&amp; {Write-Output 'Machine restarted.'}\""
  }
  ...
  #   script           = "./packer/powershell/packages/mRemoteNG.ps1"
  # }
  provisioner "powershell" {
    environment_vars = ["SAStoken=${var.SAStoken}"]
    script           = "./packer/powershell/packages/winscp.ps1"
  }
  provisioner "powershell" {
    environment_vars = ["SAStoken=${var.SAStoken}"]
  ...
  # provisioner "powershell" {
  #   environment_vars = ["SAStoken=${var.SAStoken}"]
  #   script           = "./packer/powershell/packages/putty.ps1"
  # }
  # provisioner "powershell" {
  #   environment_vars = ["SAStoken=${var.SAStoken}"]
  #   script           = "./packer/powershell/packages/lockoutstatus.ps1"
  # }
  provisioner "windows-restart" {
    restart_check_command = "powershell -command \"&amp; {Write-Output 'Machine restarted.'}\""
  }
